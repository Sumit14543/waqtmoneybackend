...
const baseUrl = "https://payday-api.waqtmoney.com";
...
async function run() {
  const found = await queryCrm({ sourceLeadId: "WAQTMN-PD-012353359423" });
  if (!found) {
    await queryCrm({ loanId: "WQTMNO01141" });
  }
  if (!found) {
    await queryCrm({ loanId: "LNWQTMN01141" });
  }
}
