...
const fetchLoanDetails = async (panNumber: string) => {
  const endpoints = [
    `https://testing-api.waqtmoney.com/api/integrations/leads/status?sourceSystem=waqtmoney&pan=${panNumber}`,
    `https://payday-api.waqtmoney.com/api/integrations/leads/status?sourceSystem=waqtmoney&pan=${panNumber}`
  ];
    ...
    try {
      const response = await fetch(url, {
        ...
        headers: {
          ...
          "x-integration-api-key": "waqt@2026",
        },
      });
      ...
      if (data) {
        sessionStorage.setItem("repaymentCrmStatus", JSON.stringify(data));
        console.log({
          loanId: data.repayment?.loanId || "-",
          dueDate: data.repayment?.dueDate 
            ? new Date(data.repayment.dueDate).toLocaleDateString("en-IN") 
            : "-",
          loanAmount: data.loanAmount ? `₹${data.loanAmount}` : "₹0",
          tenure: data.tenure_days ? `${data.tenure_days} Days` : "-",
          interestRate: data.interest_rate ? `${data.interest_rate}%` : "-",
          interestAccrued: data.interest_accrued ? `₹${data.interest_accrued}` : "-",
          paidAmount: data.repayment?.amountPaid ? `₹${data.repayment.amountPaid}` : "₹0",
          outstandingToday: data.repayment?.outstanding ? `₹${data.repayment.outstanding}` : "₹0",
          maturityAmount: data.repayment?.dueAmount ? `₹${data.repayment.dueAmount}` : "₹0"
        });
        return;
      }
    } catch (error) {
  ...
  console.error("All endpoints failed to fetch loan details.");
};
...
const Repayment = () => {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      try {
        await fetchLoanDetails(pan);
      } catch (err) {
        console.error("Direct loan details fetch failed:", err);
      }
